# Hub Engineering Principles

These are the engineering principles we currently want Hub to follow.

They are informed by external sources, but the sections below are not meant to
be a verbatim summary of those sources. This file is the original, Hub-flavored
seed kept for provenance; the generalized, template-neutral version this repo
actually adopts lives in `docs/harness/principles.md`.

Sources: [Harness Engineering](https://openai.com/index/harness-engineering/)
([local copy](harness-engineering.md)).
...
Anything the agent needs to do reliable work must live in the repo. If context
only exists in chat, docs outside the repo, or in someone's head, it is not
...
These ideas are useful and likely relevant to Hub, but they are not yet strong
enough to be treated as repo-wide rules.
...
with higher throughput. We should adopt this where it helps, while keeping human
judgment for merge decisions and product-level tradeoffs.

### 10. Doc-gardening should become a recurring maintenance loop

As the repo grows, stale docs and bad patterns will accumulate. A recurring
...
These are useful defaults once Hub has more runtime behavior to observe.

### 12. Structured logs over free-form text

If we emit logs, prefer machine-queryable fields over narrative strings.
